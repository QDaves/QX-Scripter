...
public class ABCFile : IFlashItem, IDisposable
{
    ...
    public bool HasFloatSupport =>
        Version.Major > 47 ||
        Version.Major == 47 && Version.Minor >= 16;
    ...
    private void CacheByNaming(ASClass @class)
    {
        ...
        {
            string? prefix = null;
            string? namespace_name = @class.QName.Namespace?.RuntimeName;
            if (!string.IsNullOrWhiteSpace(namespace_name) && !namespace_name.StartsWith("_-"))
            {
                prefix = namespace_name + ".";
            }
            _instanceByConstructorCache.TryAdd(prefix + @class.Instance.Constructor.Name, @class.Instance);
        }
