...
public abstract class ASContainer : IFlashItem
{
    ...
    public IEnumerable<ASMethod> GetMethods()
    {
        return GetTraits(TraitKind.Method, TraitKind.Getter, TraitKind.Setter)
            .Select(t => t.Method ?? throw new InvalidDataException($"{t.Kind} trait has no method."));
    }
    ...
    {
        foreach (ASTrait trait in GetTraits(
            TraitKind.Method,
            TraitKind.Getter,
            TraitKind.Setter))
        {
            if (qualifiedName != null && trait.QName.Name != qualifiedName) continue;
            ASMethod method = trait.Method ??
                throw new InvalidDataException($"{trait.Kind} trait has no method.");
    ...
    {
        return GetGetters()
            .Where(g => g.Method?.ReturnType?.Name == returnTypeName);
    }
    ...
    protected void PopulateTraits(ref SpanFlashReader input)
    {
        ...
        for (int i = 0; i < Traits.Capacity; i++)
        {
            ...
            {
                (trait.Method ?? throw new InvalidDataException($"{trait.Kind} trait has no method.")).Container = this;
            }
