...
public class ASInstance : ASContainer, IAS3Item
{
    ...
    public override ASMultiname QName => ABC.Pool.Multinames[QNameIndex] ??
        throw new InvalidDataException($"Instance QName constant {QNameIndex} is null.");
    ...
    public IEnumerable<ASMultiname> GetInterfaces()
    {
        for (int i = 0; i < InterfaceIndices.Count; i++)
        {
            yield return ABC.Pool.Multinames[InterfaceIndices[i]] ??
                throw new InvalidDataException($"Interface multiname constant {InterfaceIndices[i]} is null.");
        }
    }
    public bool ContainsInterface(string qualifiedName)
    {
        ASMultiname? super = Super;
        if (super?.Name is not null and not "Object")
        {
            ASInstance? superInstance = ABC.GetInstance(super);
            if (superInstance?.ContainsInterface(qualifiedName) == true)
            {
                return true;
            }
        }
    ...
    public string ToAS3()
    {
        string as3 = QName.Namespace?.GetAS3Modifiers() ?? string.Empty;
        ...
        as3 += QName.Name;
        ASMultiname? super = Super;
        string? super_name = super?.Name;
        if (!isInterface && !string.IsNullOrWhiteSpace(super_name) && super_name != "Object")
        {
            as3 += $" extends {super_name}";
        }
