...
public class ASTrait : IFlashItem, IAS3Item, IMethodGSTrait, ISlotConstantTrait, IClassTrait, IFunctionTrait
{
    ...
    public ASMultiname QName => ABC.Pool.Multinames[QNameIndex] ??
        throw new InvalidDataException($"Trait QName constant {QNameIndex} is null.");
    ...
    public object? Value => Kind is TraitKind.Slot or TraitKind.Constant
        ? ABC.Pool.GetDefaultValue(
            ValueKind,
            ValueIndex,
            Type)
        : null;
        ...
        : this(abc)
    {
        ...
        switch (Kind)
        {
            ...
            case TraitKind.Setter:
            {
                ...
                (Method ?? throw new InvalidDataException($"{Kind} trait has no method.")).Trait = this;
                break;
            }
    ...
    public string ToAS3()
    {
        if (Kind is TraitKind.Constant or TraitKind.Slot)
        {
            ...
            if (type != null)
            {
                ...
                {
                    builder.Append(".<");
                    builder.Append(string.Join(',', type.GetTypes().Select(parameter => parameter?.Name ?? "*")));
                    builder.Append('>');
                }
            }
            if (ValueIndex != 0)
