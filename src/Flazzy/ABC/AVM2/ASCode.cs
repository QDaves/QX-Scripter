...
public class ASCode : FlashItem, IList<ASInstruction>
{
    ...
    private readonly HashSet<Jumper> _unresolvedJumps;
    ...
    public ASInstruction this[int index]
    {
        ...
        set
        {
            ...
            if (Jumper.IsValid(previous.OP) && Jumper.IsValid(value.OP))
            {
                ...
                if (JumpExits.Remove(previousJumper, out ASInstruction? exit))
                {
                    JumpExits.Add(jumper, exit);
                }
                else if (_unresolvedJumps.Remove(previousJumper))
                {
                    _unresolvedJumps.Add(jumper);
                }
            }
    ...
    public ASCode(ABCFile abc, ASMethodBody body)
    {
        ...
        _unresolvedJumps = new HashSet<Jumper>();
    ...
    public static ASInstruction DecodeAt(
        ABCFile abc,
        ...
        int offset)
    {
        ...
        var input = new SpanFlashReader(code)
        {
            Position = offset
        };
        ASInstruction instruction = ASInstruction.Create(abc, ref input);
        instruction.DecodedOffset = offset;
        instruction.DecodedSize = input.Position - offset;
        if (instruction.DecodedSize <= 0 || input.Position > code.Length)
            throw new InvalidDataException(
                $"Invalid AVM2 instruction span {offset}+{instruction.DecodedSize}/{code.Length}.");
        return instruction;
    }
        ...
        if ((index + count) <= _instructions.Count)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                if (entry != null)
                {
                    ...
                    {
                        JumpExits.Remove(entry);
                        _unresolvedJumps.Add(entry);
                    }
                }
                foreach (LookUpSwitchIns @switch in SwitchExits.Keys)
                {
                    ...
                    for (int j = 0; j < exits.Length; j++)
                    {
                        ...
                        exits[j] = index < _instructions.Count
                            ? _instructions[index]
                            : null;
                    }
                }
                ...
                if (Jumper.IsValid(instruction.OP))
                {
                    ...
                    _unresolvedJumps.Remove(jumper);
                }
    ...
    public void InsertRange(int index, IEnumerable<ASInstruction> collection)
    {
        ...
        {
            var deadJumps = new Stack<Jumper>(_unresolvedJumps);
            ...
            {
                Jumper jumper = deadJumps.Pop();
                JumpExits[jumper] = collection.First();
                _unresolvedJumps.Remove(jumper);
            }
    ...
    private void LoadInstructions()
    {
        ...
        while (input.IsDataAvailable)
        {
            ...
            ASInstruction instruction = DecodeAt(
                _abc,
                _body.Code,
                checked((int)previousPosition));
            input.Position = instruction.DecodedOffset + instruction.DecodedSize;
            ...
            if (instruction.OP == OPCode.LookUpSwitch)
            {
                ...
                for (int i = 0; i < offsets.Count; i++)
                {
                    ...
                    else if (marks.TryGetValue(exitPosition - uint.MaxValue - 1, out ASInstruction? exit))
                    {
                        exits[i] = exit;
                    }
                }
            ...
            else if (Jumper.IsValid(instruction.OP))
            {
                ...
                else
                {
                    ...
                    if (marks.TryGetValue(markIndex, out ASInstruction? exit) && exit?.OP == OPCode.Label)
                        JumpExits.Add(jumper, exit);
                }
            }
        }
    ...
    private int GetFinalJumpCount(ASMachine machine, Jumper jumper, List<ASInstruction> cleaned, Dictionary<int, List<ASInstruction>> localReferences, Stack<ASInstruction> valuePushers)
    {
        ...
        if (!JumpExits.TryGetValue(jumper, out ASInstruction? exit) || exit is null)
        {
        ...
        if (isJumping == true && block != null)
        {
            ...
            foreach (KeyValuePair<LookUpSwitchIns, ASInstruction?[]> switchExit in SwitchExits)
            {
                ...
                {
                    if (caseExit is not null && block.Contains(caseExit))
                    {
                        cleaned.Add(jumper);
                        return 0;
                    }
