...
public abstract class ASInstruction : IFlashItem, ICloneable
{
    ...
    public int DecodedOffset { get; internal set; } = -1;
    public int DecodedSize { get; internal set; }
    protected ABCFile ABC => _abc ?? throw new InvalidOperationException($"{GetType().Name} has no ABC context.");
    ...
    protected ASMultiname GetMultiname(int index, string role) =>
        ABC.Pool.Multinames[index] ??
        throw new InvalidDataException($"{role} multiname constant {index} is null.");

    protected ASNamespace GetNamespace(int index, string role) =>
        ABC.Pool.Namespaces[index] ??
        throw new InvalidDataException($"{role} namespace constant {index} is null.");

    protected string GetString(int index, string role) =>
        ABC.Pool.Strings[index] ??
        throw new InvalidDataException($"{role} string constant {index} is null.");
    ...
    public static ASInstruction Create(ABCFile abc, ref SpanFlashReader input)
    {
        ...
        return op switch
        {
            ...
            OPCode.IsType => new IsTypeIns(abc, ref input),
            ...
            OPCode.PushNamespace => new PushNamespaceIns(abc, ref input),
            OPCode.PushUInt => new PushUIntIns(abc, ref input),
            OPCode.PushDouble => new PushDoubleIns(abc, ref input),
            OPCode.PushFloat => new PushFloatIns(abc, ref input),
            OPCode.PushFloat4 => new PushFloat4Ins(abc, ref input),
            ...
            OPCode.Li8 => new Li8Ins(),
            OPCode.Li16 => new Li16Ins(),
            OPCode.Li32 => new Li32Ins(),
            OPCode.Lf32 => new Lf32Ins(),
            OPCode.Lf32x4 => new Lf32x4Ins(),
            OPCode.Lf64 => new Lf64Ins(),
            OPCode.Si8 => new Si8Ins(),
            OPCode.Si16 => new Si16Ins(),
            OPCode.Si32 => new Si32Ins(),
            OPCode.Sf32 => new Sf32Ins(),
            OPCode.Sf32x4 => new Sf32x4Ins(),
            OPCode.Sf64 => new Sf64Ins(),
            OPCode.Sxi1 => new Sxi1Ins(),
            OPCode.Sxi8 => new Sxi8Ins(),
            OPCode.Sxi16 => new Sxi16Ins(),

            // Type conversion
            OPCode.Coerce_a => new CoerceAIns(),
            OPCode.Coerce_b => new CoerceBIns(),
            OPCode.Coerce_d => new CoerceDIns(),
            OPCode.Coerce_i => new CoerceIIns(),
            OPCode.Coerce_o => new CoerceOIns(),
            OPCode.Coerce => new CoerceIns(abc, ref input),
            OPCode.Coerce_s => new CoerceSIns(),
            OPCode.Coerce_u => new CoerceUIns(),
            OPCode.Convert_b => new ConvertBIns(),
            OPCode.Convert_d => new ConvertDIns(),
            OPCode.Convert_f => new ConvertFIns(),
            OPCode.Convert_f4 => new ConvertF4Ins(),
            ...
            OPCode.BkptLine => new BkptLineIns(ref input),
            OPCode.Timestamp => new TimestampIns(),
            ...
            OPCode.FindDef => new FindDefIns(abc, ref input),
            ...
            OPCode.GetGlobalSlot => new GetGlobalSlotIns(ref input),
            OPCode.GetLex => new GetLexIns(abc, ref input),
            OPCode.GetOuterScope => new GetOuterScopeIns(ref input),
            ...
            OPCode.SetGlobalSlot => new SetGlobalSlotIns(ref input),
