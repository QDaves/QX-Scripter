...
public class ShockwaveFlash : IDisposable
{
    ...
    public virtual void Disassemble(Action<TagItem>? callback)
    {
        FlashReader input = _input ??
            throw new InvalidOperationException("A template has no input stream to disassemble.");
        if (input.IsDisposed)
        {
        ...
        long position = input.BaseStream.CanSeek
            ? input.Position
            : 8 + Frame.Area.GetByteSize() + 4;
        while (position != FileLength)
        {
            if (input.BaseStream.CanSeek)
                position = input.Position;
            var header = new HeaderRecord(input);
            header.SourceOffset = position;
            position += header.SourceHeaderLength!.Value;
