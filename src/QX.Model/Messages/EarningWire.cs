...
internal static class EarningWire
{
    public const int MaximumCollectionCount = ushort.MaxValue;
    public const int MaximumStrings = MaximumCollectionCount;
    public const int MaximumStringBytes = 16 * 1024 * 1024;
    public const int StringPrefixBytes = sizeof(short);
    public const int EntryMinimumBytes = sizeof(byte) * 2 + sizeof(int) + StringPrefixBytes;
...
internal struct EarningStringBudget
{
    ...
    public EarningStringBudget(int maximum_count, int maximum_bytes)
    {
    ...
    public string Read(in PacketReader p, string name, int trailing_bytes)
    {
        EarningWire.RequireRemaining(
            in p,
            EarningWire.StringPrefixBytes,
            trailing_bytes,
            name);
        int byte_count = unchecked((ushort)p.ReadShort());
        EarningWire.RequireRemaining(in p, byte_count, trailing_bytes, name);
